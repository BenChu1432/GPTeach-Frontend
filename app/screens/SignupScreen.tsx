...
import {signIn, signup} from "../api/FirebaseResources"
...
export default function SignupScreen({navigation}) {
    ...
    const [signupError,setSignupError]=useState<boolean|undefined>(undefined);

    const validationSchema=Yup.object().shape({
        email: Yup.string().required().email().label("Email"),
        password: Yup.string().required().min(4).label("Password"),
        confirmPassword:Yup.string().oneOf([Yup.ref('password'), undefined], 'Passwords must match').required().label("Password")
    })

    const handleSignin=(email:string, password:string)=>{
        signup(email, password,setLoading,setSignupError);
    }
